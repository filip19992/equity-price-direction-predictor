...
LEGACY_DATASET_DIR = NOTEBOOK_DIR.parent / "data" / "equity_data" / "model_data"
DEFAULT_DATASET_NAME = "stock_direction_two_tickers_base.csv"
...
def locate_panel_dataset(dataset_name: str = DEFAULT_DATASET_NAME) -> Path:
    candidates = [
        ...
        LEGACY_DATASET_DIR / dataset_name,
    ]
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError(
        "Two-ticker base dataset not found. Checked: "
...
def add_reddit_block(frame: pd.DataFrame, features: pd.DataFrame, prefix: str) -> None:
    ...
    if vader_col in frame.columns:
        features[f"{prefix}vader_mean"] = shifted_series(frame, vader_col, fill="zero")

    if finbert_col in frame.columns:
        features[f"{prefix}finbert_mean"] = shifted_series(frame, finbert_col, fill="zero")
...
def engineer_ticker_features(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    ret_5d = pd.to_numeric(work["stock_return_5d"], errors="coerce")
    ...
    features["gdelt_articles_z20"] = rolling_zscore(log_articles, 20)
...
def build_feature_panel(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    feature_cols = [col for col in features.columns if col not in ["date", "ticker", "company_name", "future_return_1d", "y", "y_code", "y_dir"]]
